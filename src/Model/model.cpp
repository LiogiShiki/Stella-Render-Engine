...
void Model::loadModel(std::string path)
{
    ...
    if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode)
    {
        ...
        str << "ERROR LOADING ASSIMP MODEL (" << path << ") ||  ASSIMP ERROR : " << importer.GetErrorString();
        std::cout << str.str();
        return;
    }
...
void Model::Draw(Lumen::Shader &shader, std::shared_ptr<Camera> p_Camera,const  bool textured)
{
    shader.Use();
    shader.SetMatrix4("model", m_Model);
    shader.SetMatrix4("prevModel", m_Model); // static object
    shader.SetVector3f("camPos", p_Camera->Position);
...
void Model::drawShadow(Lumen::Shader& shader)
{
    shader.Use();
    shader.SetMatrix4("model", m_Model);
    for (int i = 0; i < meshes.size(); ++i) {
...
void Model::DrawRSMGbuffer(Lumen::Shader& shader)
{
    shader.Use();
    shader.SetMatrix4("model", m_Model);
    for (int i = 0; i < meshes.size(); ++i) {
        meshes[i].DrawRSMGbuffer(shader);
    }
}
