...
namespace Lumen
{
	...
	{
	public :
		Framebuffer(unsigned int w, unsigned int h, std::vector<FORMAT> format, bool create_on_construct, bool has_depth_attachment,bool cube_attachment);
		Framebuffer(unsigned int w, unsigned int h, FORMAT format, bool create_on_construct, bool has_depth_attachment, bool cube_attachment);
		~Framebuffer();
		...
		Framebuffer(Framebuffer&& v) : m_HasDepthMap(v.m_HasDepthMap), m_IsCubeAttachment(v.m_IsCubeAttachment)
		{
			m_FBO = v.m_FBO;
