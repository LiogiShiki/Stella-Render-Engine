...
namespace Lumen
{
    Framebuffer::Framebuffer(unsigned int w, unsigned int h, std::vector<FORMAT> format, bool create_on_construct, bool has_depth_attachment, bool cube_attachment) :
        m_FBO(0), m_FBWidth(w), m_FBHeight(h), m_HasDepthMap(has_depth_attachment), m_Format(format),m_IsCubeAttachment(cube_attachment)
    {
    ...
    Framebuffer::Framebuffer(unsigned int w, unsigned int h, FORMAT format,
                            bool create_on_construct, bool has_depth_attachment, bool cube_attachment) :
        m_FBO(0), m_FBWidth(w), m_FBHeight(h), m_HasDepthMap(has_depth_attachment), m_Format({ format }),m_IsCubeAttachment(cube_attachment)
    {
        if (create_on_construct)
        {
	...
	void Framebuffer::CreateFramebuffer()
	{
        ...
        {
            Logger::LogToFile("Fatal error! Framebuffer creation failed!");
        }
